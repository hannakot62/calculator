...
function SuperHighPriorityOperation(action, index) {// выражения вида 76! 3^(+/-4)
    let result;
    switch (action[index]) {
        case '!': {
    ...
    while (task.includes('!') || task.includes('^')) {
        ...
        switch (task[superHighIndex]) {
            case '!': {
                ...
                task = task.split('');
                task.splice((superHighIndex - baseLength), symbolsToDelete, SuperHighResult);
                task = task.join('');
                break;
            }
            case '^': {
                if (task[superHighIndex + 1] === '(') {//если показатель отрицательный
    ...
    while (operationsLeft) {
        ...
        for (let i = 1; operationFound < 2; i++) {
            if (!(Number.isInteger(+task[i]) || task[i] === '.')) {
                operationFound++;
                if (operationFound != 2) { simpleTask += task[i]; }
            }
            else {
                simpleTask += task[i];
            }
...
function BracketCalculationPart(task) {
    while (task.includes('(')) {
        ...
        if (!(Number(action) < 0 && //если в скобках НЕ отрицательный правый операнд приоритетной операции
            ...
            if ((action[0] === '-' && task[LeftBracketIndex - 1] != '(')) {
                task = task.split('');
                if (action[0] === '-' && (task[LeftBracketIndex - 1] === '^' ||//проверка на отрицательный второй операнд для */^%
                    task[LeftBracketIndex - 1] === '/' ||
                    task[LeftBracketIndex - 1] === '*' ||
                    task[LeftBracketIndex - 1] === '%')) {
            ...
            if (task.slice(0, LeftBracketIndex).includes('(')) {
                ...
                let outerAction = task.slice((outerLeftBracketIndex + 1), (outerRightBracketIndex));// действие без скобок
                ...
                if ((outerAction[0] === '-' && task[outerLeftBracketIndex - 1] != '(')) {
                    task = task.split('');
                    if (outerAction[0] === '-' && (task[outerLeftBracketIndex - 1] === '^' ||//проверка на отрицательный второй операнд для */^%
